...
},{"./style.css":20,"./x-controller.js":22,"bootstrap":3,"jquery":18}],3:[function(require,module,exports){
...
module.exports = function () {
    ...
    $(document).ready(function () {
        ...
        $('#x-reset').click(function () {
            var inputCount = xor.inputCount,
        ...
        $('#x-step').click(function () {
            currentStep += 1;
            var trainEntry = trainSet[(currentStep - 1) % trainSet.length];
            xor.train(trainEntry[0], trainEntry[1]);
            update();
        });

        $('#x-run').click(function () {
        ...
        $('#x-neurons').change(function () {
            xor.resize($('#x-neurons').val());
            update();
        });

        $('#x-lambda').change(function () {
            xor.lambda = parseFloat($('#x-lambda').val());
            update();
        });

        $('#x-speed').change(function () {
            xor.speed = parseFloat($('#x-speed').val());
            update();
        });

        $('.x-answer').change(function (e) {
            var i = parseInt($(e.target).data('x-entry-id')),
                val = parseInt($(e.target).val());
            trainSet[i][1] = val;
            update();
        });

        var determineOperation = function determineOperation() {
            if (trainSet[0][1] == -1 && trainSet[1][1] == 1 && trainSet[2][1] == 1 && trainSet[3][1] == -1) {
                $('.x-operation').text('⊕');
        ...
        var evaluate = function evaluate() {
            return trainSet.map(function (entry) {
                return Math.abs(xor.calc(entry[0]) - entry[1]) < config.eps;
            }).every(function (x) {
                return x == true;
            });
        };
        ...
        var update = function update() {
            ...
            var activationFunctionPoints = [];
            for (var x = -1; x <= 1; x += 0.05) {
                var y = 2 / (1 + Math.exp(-xor.lambda * x)) - 1;
...
},{"./config.js":1,"./x-graph-controller.js":23,"./xor-problem.js":24,"underscore":19}],23:[function(require,module,exports){
'use strict';
...
var GraphConroller = function () {
    function GraphConroller(xorNN, id) {
        _classCallCheck(this, GraphConroller);
    ...
    _createClass(GraphConroller, [{
        ...
        value: function draw() {
            ...
            for (var i = 0; i < hiddenCount; i++) {
                var offsetFactor = hiddenCount == 1 ? 0.5 : i / (hiddenCount - 1);
            ...
            h.forEach(function (point, i) {
                _this.ctx.beginPath();
                _this.ctx.arc(point.x, point.y, radius, 0, 2 * Math.PI);
                _this.ctx.fillText("h" + (i + 1), point.x + 1.2 * radius, point.y - 1.2 * radius);
                _this.ctx.stroke();
            });
            ...
            inputs.forEach(function (input) {
                h.forEach(function (hidden) {
                    _this.ctx.moveTo(input.x + radius, input.y);
                    _this.ctx.lineTo(hidden.x - radius, hidden.y);
                });
            });
            h.forEach(function (hidden) {
                _this.ctx.moveTo(hidden.x + radius, hidden.y);
                _this.ctx.lineTo(f.x - radius, f.y);
            });
            this.ctx.stroke();

            /* Weights */
            var inputLabels = ['a', 'b', '1'],
                hiddenWeightsHtml = this.xor.hiddenNeurons.map(function (n, i) {
                return n.map(function (w, j) {
                    return '<i>W<sub>' + inputLabels[j] + 'h' + (i + 1) + '</sub> = ' + w.toFixed(2) + '</i>';
                }).join('</br>');
            }).join('</br>'),
                outputWeightsHtml = this.xor.outputWeights.map(function (w, i) {
                return '<i>W<sub>h' + (i + 1) + 'o</sub> = ' + w.toFixed(2) + '</i>';
            }).join('</br>');
            ...
            /* Adjustment */
            var outputAdjustments = this.xor.trainLog.outputAdjustments || [],
    ...
    return GraphConroller;
}();

module.exports = GraphConroller;

},{"./config.js":1,"flot":17,"underscore":19}],24:[function(require,module,exports){
'use strict';
...
var XorNN = function () {
    function XorNN(inputCount, hiddenNeuronsCount, lambda, speed) {
        var _this = this;

        _classCallCheck(this, XorNN);
        ...
        _(hiddenNeuronsCount).times(function () {
            var weghts = [];
            _(inputCount).times(function () {
                return weghts.push(util.random(-5, 5));
            });
            _this.hiddenNeurons.push(weghts);
            _this.outputWeights.push(util.random(-5, 5));
        });
    }

    _createClass(XorNN, [{
        key: 'activate',
        value: function activate(input, weights) {
            var net = input.map(function (val, i) {
        ...
        key: 'calc',
        value: function calc(input) {
            var _this2 = this;

            var hiddenOutputs = this.hiddenNeurons.map(function (n) {
                return _this2.activate(n, input);
            });
            return this.activate(hiddenOutputs, this.outputWeights);
        }
    }, {
        key: 'train',
        value: function train(input, expectedValue) {
            var _this3 = this;

            var f = util.formatNumber,
                hiddenOutputs = this.hiddenNeurons.map(function (n) {
                return _this3.activate(n, input);
            }),
        ...
        key: 'resize',
        value: function resize(neurons) {
            var _this4 = this;

            var curNeurons = this.hiddenNeuronsCount;
            console.log(curNeurons, neurons);
            if (neurons > curNeurons) {
                _(neurons - curNeurons).times(function () {
                    var weights = [];
                    _(_this4.inputCount).times(function () {
                        return weights.push(util.random(-5, 5));
                    });
                    _this4.hiddenNeurons.push(weights);
                    _this4.outputWeights.push(util.random(-5, 5));
                });
            } else {
...
},{"./config.js":1,"./util.js":21,"underscore":19}]},{},[2]);
